        ...
        with BetaContext.write_session() as sess:
            active_instruments = list(
                sess.scalars(
                    ...
                    .order_by(BetaUniverseMembership.priority_rank.asc().nulls_last(), BetaInstrument.symbol.asc())
                ).all()
            )
            ...
            for instrument in active_instruments:
                ...
                if catalog_row is not None and instrument.core_security_id is None:
                    instrument.metadata_json = json.dumps(
                        {
                            "catalog_isin": catalog_row.isin or "",
                            "catalog_figi": catalog_row.figi or "",
                        },
                        sort_keys=True,
                    )
